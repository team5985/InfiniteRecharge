...
public class RightTrenchAllBallRepositionAuto extends AutoMode {
    private String name = "Right Trench All Ball Auto";
    private AutoSelection autoType = AutoSelection.RIGHT_TRENCH_ALL_BALL_REPOSITION_AUTO;
    ...
    public boolean runStep(int step) {
        switch(step) {
            ...
            Drive.getInstance().actionSensorDrive(1.0, 0.0, 6.25);

            System.out.println("ENCODER:" + Drive.getInstance().getAvgEncoderDistance());
            if (Drive.getInstance().getAvgEncoderDistance() >= 6.24) {
                return true;
            }
            break;
